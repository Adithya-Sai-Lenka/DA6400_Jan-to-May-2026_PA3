...
class SACAgent:
    def __init__(self, obs_dim, action_dim, device, gamma=0.99, tau=0.005, lr=3e-4, alpha=0.2):
        self.device = device
        self.gamma = gamma
        self.tau = tau
        self.alpha = alpha
...
def train_target_pendulum(target_angle, alpha, seed, num_steps=80000, random_steps=2000, eval_freq=5000):
    set_seed(seed)
    ...
    agent = SACAgent(obs_dim, action_dim, device, gamma=0.99, alpha=alpha)
    ...
    for step in range(0, num_steps + 1):
        ...
        if step % eval_freq == 0:
            ...
            print(f"Angle: {target_angle}° | Alpha: {alpha:5.3f} | Seed: {seed:2d} | Step: {step:5d} | Eval Avg Return: {eval_reward:.2f}")

    env.close()
    return eval_metrics

def run_grid_task(args):
    """Helper function to unpack arguments for the parallel executor."""
    angle, alpha, seed, total_steps, eval_freq = args
    ...
    return angle, alpha, seed, metrics
...
if __name__ == "__main__":
    ...
    target_angles = [-60, 90, 120, -150]
    alpha_values = [0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.4] 
    num_seeds = 15
    eval_freq = 5000 
    total_steps = 80000 
    
    # 3D Nested dictionary to aggregate logs: [angle][alpha][seed]
    experiment_logs = {
        angle: {alpha: {} for alpha in alpha_values} 
        for angle in target_angles
    }
    
    # Prepare the list of all 300 tasks
    tasks = []
    for angle in target_angles:
        for alpha in alpha_values:
            for seed in range(1, num_seeds + 1):
                tasks.append((angle, alpha, seed, total_steps, eval_freq))
            
    # max_workers = min(mp.cpu_count() - 1, len(tasks))
    max_workers = 15
    
    print(f"{'='*50}")
    print(f"Starting Grid Search Evaluation")
    print(f"Total Tasks: {len(tasks)} | Max Workers: {max_workers}")
    print(f"{'='*50}\n")
    
    # Execute tasks in parallel
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(run_grid_task, task) for task in tasks]
        
        # Wrap the completed futures iterator with tqdm
        for future in tqdm(concurrent.futures.as_completed(futures), total=len(futures), desc="Grid Search Progress"):
            try:
                angle, alpha, seed, metrics = future.result()
                experiment_logs[angle][alpha][seed] = metrics

                tqdm.write(f"✅ Completed -> Angle: {angle:4d}° | Alpha: {alpha:5.3f} | Seed: {seed:2d}")
                
            except Exception as exc:
                tqdm.write(f"❌ Task generated an exception: {exc}")
                
    print("\nAll grid search evaluations completed successfully.")
    np.save('sac_pendulum_manual_temp_tuning_eval_results.npy', experiment_logs, allow_pickle=True)